...
/* ========================================================= STATE ========================================================= */ let characters = []; let team = []; let selectedTeamSlot = null;
...
/* ========================================================= INIT ========================================================= */ document.addEventListener("DOMContentLoaded", () => { fetch("data/characters.json") .then(r => r.json()) .then(data => { characters = data.map(c => ({ name: c.name, element: c.element, class: c.class, position: c.position, image: c.image && c.image.trim() ? c.image : FALLBACK_IMG }));

loadFromURLorStorage();
  normalizeTeam(); // ⬅️ HANYA DI SINI
...
/* ========================================================= FILTER ========================================================= */ function setupFilters() { document.querySelectorAll(".filter-btn").forEach(btn => { btn.addEventListener("click", () => { const type = btn.dataset.type; const value = btn.dataset.value;
...
/* ========================================================= CHARACTERS LIST ========================================================= */ function renderCharacters() { charsEl.innerHTML = "";
...
/* ========================================================= TEAM LOGIC ========================================================= */ function toggleCharacterInTeam(character) { // REMOVE JIKA SUDAH ADA const existingIndex = team.findIndex(t => t && t.name === character.name);

if (existingIndex !== -1) { animateSlot(existingIndex, () => { team[existingIndex] = null; persist(); updateURL(); renderTeam(); }); return; }

// JIKA SLOT SUDAH DIPILIH if (selectedTeamSlot !== null) { const target = selectedTeamSlot;

animateSlot(target, () => {
  team[target] = character;
  clearSelection();
  persist();
  updateURL();
  renderTeam();
...
// DEFAULT: SLOT KOSONG PERTAMA const emptyIndex = team.findIndex(t => t === null); if (emptyIndex === -1) { alert("Max 5 characters"); return; }

animateSlot(emptyIndex, () => { team[emptyIndex] = character; persist(); updateURL(); renderTeam(); }); }
...
function handleSlotClick(index, slotEl) { // REMOVE JIKA ADA ISI if (team[index]) { animateSlot(index, () => { team[index] = null; persist(); updateURL(); renderTeam(); }); return; }

// SELECT SLOT KOSONG clearSelection(); selectedTeamSlot = index; slotEl.classList.add("selected-slot"); }

/* ========================================================= SYNERGY WARNING ========================================================= */ let synergyWarningEls = [];
...
const onlyKnightWizardArcher = classes.every(c => ["Knight", "Wizard", "Archer"].includes(c) );

const onlyWizardArcherHealer = classes.every(c => ["Wizard", "Archer", "Healer"].includes(c) );

const onlyWizardArcher = classes.every(c => ["Wizard", "Archer"].includes(c) );
...
if (onlyWizardArcherHealer && !hasKnight) {
  warnings.push("Anda tidak memiliki unit frontline (Knight) untuk menahan serangan");
}
...
/* ========================================================= ANIMATION & HELPERS ========================================================= */ function animateSlot(index, callback) { const el = teamEl.children[index]; if (!el) return callback();
...
function clearSelection() { selectedTeamSlot = null; document.querySelectorAll(".selected-slot").forEach(el => el.classList.remove("selected-slot") ); }

/* ========================================================= STORAGE & SHARE ========================================================= */ function persist() { localStorage.setItem("team", JSON.stringify(team)); }
